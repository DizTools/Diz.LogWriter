...
internal class LogCreatorTempLabelGenerator
{
    ...
    private Branch? TryGeneratePlusMinusLabelAtOffset(ISnesData snesData, int sourceOffset)
    {
        ...
        var destOffset = destSnesAddr == null ? -1 : Data.ConvertSnesToPc((int)destSnesAddr.Value);
        if (destOffset == -1) 
            return null;
        ...
        var existingLabel = Data.TemporaryLabelProvider.GetLabel((int?)destSnesAddr ?? -1);
    ...
    private void GenerateTempLabelIfNeededAt(int originOffset)
    {
        ...
        {
            var snesDestinationIa = (int?)Data.GetIntermediateAddressOrPointer(originOffset) ?? -1;
