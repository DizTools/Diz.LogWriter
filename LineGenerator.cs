...
public class LineGenerator
{
    ...
    private string GenerateLine(int offset, bool forceGenerateFullLine = true, string overrideFormatterName = null)
    {
        var columnTokens = LogCreatorLineFormatter.ColumnFormats
            .Select(columnFormat => new ColumnTokens {
                ColumnFormat = columnFormat, 
                Tokens = GenerateColumn(offset, columnFormat, overrideFormatterName)
            })
            .ToList();
        ...
        // concatenate prepend comments lines, if any
        var prependLineText = prependCommentsLines
            .Aggregate("", (current, prependCommentLine) => current + $"; {prependCommentLine}\n"
        );
        
        // final line(s) to write
        return prependLineText + mainLineText;
    }

    private TokenBase[] GenerateColumn(int offset, LogCreatorLineFormatter.ColumnFormat columnFormat, string overrideFormatterName = null)
    {
        ...
        return GenerateColumnTokensFromFormatter(offset, formatter);
    }

    private TokenBase[] GenerateColumnTokensFromFormatter(int offset, LogCreatorLineFormatter.ColumnFormat columnFormat)
    {
        var columnGenerator = GetGeneratorFor(columnFormat.Value);
        var sanitizedOffset = columnFormat.SanitizeOffset(offset);
        
        return columnGenerator.GenerateTokens(sanitizedOffset, columnFormat.LengthOverride);
    }
